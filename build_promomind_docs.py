...
MD_PATH = OUT_DIR / "PromoMind_bilingual_proposal.md"
DOCX_PATH = OUT_DIR / "PromoMind_bilingual_proposal.docx"
MEETING_PATH = OUT_DIR / "PromoMind_5_second_meeting_message.md"
...
SOURCES = [
    (
        ...
        "Dataset overview, one-year household transactions, table descriptions, full transaction and promotion sizes.",
    ),
    (
        "completejourney package reference manual",
        "https://cran.r-project.org/web/packages/completejourney/refman/completejourney.html",
        "Official row counts and field descriptions for package data tables.",
    ),
    (
        "completejourney GitHub repository",
        "https://github.com/bradleyboehmke/completejourney",
        "Repository overview and access instructions for full transactions and promotions.",
    ),
    (
        "implicit documentation",
        "https://benfred.github.io/implicit/",
        "ALS, BPR, item-item models, multi-threaded training, and CUDA support for ALS/BPR.",
    ),
    (
        "RecBole LightGCN documentation",
        "https://recbole.io/docs/recbole/recbole.model.general_recommender.lightgcn.html",
        "LightGCN neighborhood aggregation over the user-item graph and pairwise training mode.",
    ),
    (
        "Cornac JMLR paper",
        "https://www.jmlr.org/papers/v21/19-805.html",
        "Cornac as a framework for multimodal recommendation with auxiliary information.",
    ),
    (
        "Cornac quickstart",
        "https://cornac.readthedocs.io/en/v2.2.1/user/quickstart.html",
        "Cornac experiment workflow for datasets, models, metrics, and model comparison.",
    ),
]


FIVE_SECOND_CN = (
    "我的提议是 PromoMind：一个面向超市零售的促销感知购物篮和优惠券推荐系统，"
    "用家庭历史购物、商品信息、促销和优惠券数据预测下一次最可能购买的商品，并做商业效用重排序。"
    "完整版在这里：[粘贴共享文档链接]。线上会议里如果大家对这个提议有疑问，可以直接问问题，我就不再重复总结一遍。"
)

FIVE_SECOND_EN = (
    "My proposal is PromoMind: a promotion-aware grocery basket and coupon recommender for supermarket retail. "
    "It predicts each household's next likely purchases and re-ranks recommendations using promotion, coupon, "
    "discount cost, and business utility signals. Full proposal: [paste shared document link]. "
    "In the online meeting, please just ask questions about the proposal; I will not repeat the full summary."
)
...
def set_cell_text(cell, text, bold=False, color="1F2937"):
    cell.text = ""
    p = cell.paragraphs[0]
...
def add_table(doc, rows, header_fill="EAF3F8"):
    ...
    for i, row in enumerate(rows):
        for j, value in enumerate(row):
            ...
            if i == 0:
                shade_cell(table.cell(i, j), header_fill)
    doc.add_paragraph()
    return table


def add_callout(doc, title, body, fill="F4F7FB"):
    ...
    run.font.color.rgb = RGBColor(20, 83, 112)
    run.font.name = "Arial"
    run._element.rPr.rFonts.set(qn("w:eastAsia"), "Microsoft YaHei")
    ...
    run2.font.color.rgb = RGBColor(31, 41, 55)
...
def style_document(doc):
    ...
    for name, size, color in [
        ("Title", 23, "0F3342"),
        ("Heading 1", 15.5, "145370"),
        ("Heading 2", 12.5, "145370"),
    ]:
        style = styles[name]
...
def add_section(doc, heading, paragraphs):
    ...
    for text in paragraphs:
        ...
        if text.startswith("MVP ") or text.startswith("Risk ") or text.startswith("风险") or text.startswith("准确性") or text.startswith("系统性质") or text.startswith("商业指标") or text.startswith("需要严谨"):
...
def build_docx():
    ...
    r = title.add_run("PromoMind")
    r.bold = True
    r.font.size = Pt(28)
    r.font.color.rgb = RGBColor(15, 51, 66)
    r.font.name = "Arial"
    r._element.rPr.rFonts.set(qn("w:eastAsia"), "Microsoft YaHei")

    subtitle = doc.add_paragraph()
    subtitle.alignment = WD_ALIGN_PARAGRAPH.CENTER
    rs = subtitle.add_run("Promotion-aware Grocery Basket and Coupon Recommender\n促销感知型家庭购物篮与优惠券推荐系统")
    rs.font.size = Pt(13)
    rs.font.color.rgb = RGBColor(72, 92, 104)
    rs.font.name = "Arial"
    rs._element.rPr.rFonts.set(qn("w:eastAsia"), "Microsoft YaHei")

    meta = doc.add_paragraph()
    meta.alignment = WD_ALIGN_PARAGRAPH.CENTER
    rm = meta.add_run("Structured bilingual proposal | Recommendation Systems Project")
    rm.font.size = Pt(10)
    rm.font.color.rgb = RGBColor(92, 108, 116)
    rm.font.name = "Arial"
    doc.add_paragraph()

    add_callout(doc, "5 秒会议版 / 5-second meeting note", FIVE_SECOND_CN + "\n\n" + FIVE_SECOND_EN, "EAF3F8")
    add_table(doc, SUMMARY_TABLE)

    doc.add_page_break()
    doc.add_heading("中文完整版", level=1)
    for heading, paragraphs in CN_SECTIONS:
        add_section(doc, heading, paragraphs)

    doc.add_page_break()
    doc.add_heading("English Full Proposal", level=1)
    for heading, paragraphs in EN_SECTIONS:
        add_section(doc, heading, paragraphs)

    doc.add_heading("References / 参考资料", level=1)
    for idx, (name, url, note) in enumerate(SOURCES, start=1):
        p = doc.add_paragraph()
        r = p.add_run(f"[{idx}] {name}. {note} {url}")
        r.font.size = Pt(9)
        r.font.name = "Arial"
        r._element.rPr.rFonts.set(qn("w:eastAsia"), "Microsoft YaHei")
...
def build_markdown():
    ...
    for heading, paragraphs in CN_SECTIONS:
        ...
        for text in paragraphs:
            lines.append(f"- {text}")
        lines.append("")
    lines += ["## English Full Proposal", ""]
    for heading, paragraphs in EN_SECTIONS:
        lines += [f"### {heading}", ""]
        for text in paragraphs:
            lines.append(f"- {text}")
        lines.append("")
    lines += ["## References / 参考资料", ""]
    for idx, (name, url, note) in enumerate(SOURCES, start=1):
        lines.append(f"{idx}. [{name}]({url}) - {note}")
