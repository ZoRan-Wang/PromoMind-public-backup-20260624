...
- `NDCG@10`: 0.3255 vs. 0.1489
- `Recall@10`: 0.4105 vs. 0.1570

Details are in `docs/coupon_response_improvement.md`.

Optional exploratory switches `--wide-search`, `--search-score-blend`, `--search-rank-fusion`, `--use-value-features`, `--use-coupon-family-features`, `--use-redemption-features`, `--use-content-features`, `--use-response-priors`, and `--use-derived-features` are available for ablation, but the default XGBoost command above is the current held-out best.
